# Khởi tạo quân thể (Initialize population) với N cá thể
import numpy as np
import random
def initialize_population(num_individuals, l, distribution):
    """
    ...
    population = []
    for i in range(num_individuals):     # Loop through each individual
...
def evaluation_fitness(individual: np.ndarray, _type="1MAX", k=5):
    """
    ...
    if _type=="1MAX":
        ...
        print("hàm tối ưu không xác định !")
        exit(0)
    return fitness

def average_evaluation(population: np.ndarray, optimized_function):
    """
    - Description: Đánh giá độ thích nghi của quần thể bằng hàm OneMax
    - Arguments:
        + population: quần thể hiện tại (danh sách các cá thể)
        +  optimized_function hàm tối ưu lựa chọn
    - Return values:
    ...
    for individual in population:
        current_fitness = evaluation_fitness(individual, optimized_function)
        number_evals += 1
...
def pop_pool(population: np.ndarray, offspring: np.ndarray):
    ...
    """
    combine_poppulation=np.concatenate([population,offspring])
    return combine_poppulation
def tournament_selection(population: np.ndarray, tournament_size:int, optimized_function:str):
    ...
    for time in range(times): 
        ...
        for (low, high) in tournaments:
            ...
            best_fitness = evaluation_fitness(population[index_best_individual], optimized_function)
            for i in range(low, high+1):
                current_fitness = evaluation_fitness(population[i])
