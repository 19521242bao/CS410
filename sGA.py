# Usage:
"""
    python sGA.py -pop_size 32 -pro_size 40 -tour_size 4 -func 1MAX -crossover 1X -seed 2
"""
...
def genetic_algorithm(initialized_population, optimized_function, crossover_way, threshold=0.5, tournament_size=4, limit_steps=1e20, display=False):
    ...
    if evaluation_fitness(population[0]) == len(population[0]):
...
def main(args):
    ...
    initial_population = initialize_population(num_individuals=args['population_size'], l=args['problem_size'], distribution=DISTRIB)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed_value', '-seed', type=int, required=False, help='The  random value you choose')
